...
function SingleProduct() {
  ...
  const product = productState.default.find((item) => item._id === productId);
  const {
    _id,
    name,
    description,
  ...
  const handleAddToCart = async () => {
    if (token) {
      if (!disabled) {
        if (itemFind) {
          const newCart = await updateCartQty(cartState, _id, "increment", alertDispatch);
          cartDispatch({
          ...
          <div className="card-product-details px-1">
            <div className="card-product-single">
              <div>
                ...
                <p className="card-product-description fs-1-25 px-0">{description}</p>
                <div className="card-product-price p-0">
                  <span className="card-product-newprice fs-1-25 fw-600">
                    ₹{newprice}
                  </span>
                  {actualprice && (
                    <span className="card-product-actualprice fs-1">₹{actualprice}</span>
                  )}
                  {actualprice && (
                ...
                <span className="color-success-dark fs-0-8">Inclusive of all taxes</span>
